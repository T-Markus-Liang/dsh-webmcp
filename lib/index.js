...
import { aggregateReadiness } from './readiness.mjs'
...
class BrowserSession {
   ...
   */
  /** v1.1.0 fifth surface: /.well-known/ self-description endpoints. */
  async _probeWellKnown(rawUrl) {
    let origin
    try { origin = new URL(rawUrl).origin } catch { return [] }
    const out = []
    for (const path of ['/.well-known/webmcp', '/.well-known/mcp.json']) {
      try {
        const ctrl = new AbortController()
        const timer = setTimeout(() => ctrl.abort(), 5000)
        const res = await fetch(origin + path, { signal: ctrl.signal, headers: { accept: 'application/json' } })
        clearTimeout(timer)
        if (!res.ok) continue
        const data = await res.json()
        const list = Array.isArray(data) ? data
          : (Array.isArray(data.tools) ? data.tools
          : (data.webmcp && Array.isArray(data.webmcp.tools) ? data.webmcp.tools : null))
        if (!list) continue
        for (const t of list) {
  ...
  discover(url, { refresh = false } = {}) {
    return this._enqueue(async () => {
      ...
      try {
        ...
        // v1.1.0: fifth surface — /.well-known/ self-description endpoints.
        const extra = await this._probeWellKnown(url)
        if (extra.length) tools = tools.concat(extra.filter((t) => !tools.some((x) => x.name === t.name)))
        // Manifest drift: compare with the previous discovery of this URL.
        const names = tools.map((t) => t.name)
        const prev = this.lastManifests.get(url)
        const drift = prev ? diffManifests(prev.names, names) : null
        this.lastManifests.set(url, { names, byName: new Map(tools.map((t) => [t.name, t])) })
        const out = {
          ok: true,
          url: data.url || url,
          title: data.title || '',
  ...
  invoke(url, tool, args, { refresh = false, confirm = false } = {}) {
    return this._enqueue(async () => {
      const startedAt = Date.now()
      // v1.1.0 destructive guard — host-side, cannot be bypassed by page JS.
      // Worst-case default per MCP spec: only enforced when the annotation is
      // KNOWN destructive (baseline from a prior discover of this URL).
      const baseline = this.lastManifests.get(url)
      const ann = baseline && baseline.byName ? (baseline.byName.get(tool) || {}).annotations || null : null
      if (ann && ann.destructiveHint === true && !confirm) {
        return {
          ok: false,
          ...
          tool,
          annotations: ann,
          message: 'tool "' + tool + '" is annotated destructiveHint:true; re-invoke with confirm:true to proceed',
        }
      }
      try {
        const nav = await this._goto(url, { refresh })
        ...
        if (ann) out._meta.annotations = ann
        ...
        if (payload && payload.ok && payload.result !== undefined) {
          try {
            // v1.2.0 structured-preserving truncation: first trim long
            // human-text fields, keeping structured fields intact; only if
            // still over budget do we fall back to a clipped envelope.
            const HUMAN_FIELDS = ['text', 'description', 'message', 'summary']
            const budget = this.cfg.maxResultChars
            let serialized = JSON.stringify(payload.result)
            let trimmedHuman = false
            if (payload.result && typeof payload.result === 'object' && serialized.length > budget) {
              const clone = JSON.parse(serialized)
              for (const k of HUMAN_FIELDS) {
                if (typeof clone[k] === 'string' && clone[k].length > Math.floor(budget * 0.5)) {
                  clone[k] = clone[k].slice(0, Math.floor(budget * 0.5)) + '…'
                  trimmedHuman = true
                }
              }
              ...
              if (after.length <= budget) {
                out.result = clone
                if (trimmedHuman) out._meta.truncatedHumanFields = true
                serialized = after
              }
            }
            if (serialized.length > budget) {
              out.result = {
                ...
                preview: serialized.slice(0, budget),
  ...
  /** v1.2.0: agent-readiness over this session's last-discovered tools. */
  readiness() {
    const pool = []
    for (const { byName } of this.lastManifests.values()) {
      for (const [name, meta] of byName) pool.push({ name, ...(meta || {}) })
    }
    return pool
  }
    ...
    return {
      ...
      parameters: {
        ...
        properties: {
          ...
          refresh: { type: 'boolean', description: '跳过会话复用强制重新导航（默认 false）' },
          confirm: { type: 'boolean', description: '破坏性工具确认闸：annotations.destructiveHint 为 true 的工具必须显式传 true（默认 false）' },
        },
        required: ['url', 'tool'],
      },
      output: { schema: JSON_OUT, render: jsonRender },
      execute: async ({ url, tool, args, refresh = false, confirm = false }) => {
        const blocked = gateUrl(url)
        if (blocked) return blocked
        return pool.run(url, (s) => s.invoke(url, tool, args, { refresh, confirm }))
      },
    }
  }
        ...
        hostCtx.webServer.register({
          ...
          handler: (_req, res) => {
            ...
            res.end(renderDashboard({
              ...
              readiness: aggregateReadiness(pool.readiness()),
            }))
          },
        }),
        hostCtx.webServer.register({
          ...
          handler: (req, res) => {
            ...
            req.on('end', () => {
              ...
              const { url, tool, args, confirm = false } = payload
              res.writeHead(200, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' })
              if (typeof url !== 'string' || typeof tool !== 'string' || !tool) {
                res.end(JSON.stringify({ ok: false, error: 'bad-url', message: 'url + tool (strings) required' }))
                return
              }
              if (!/^https?:\/\//i.test(url)) {
                res.end(JSON.stringify({ ok: false, error: 'bad-url', message: 'url must start with http(s)://' }))
                return
              }
              const blocked = gateUrl(url)
              if (blocked) { res.end(JSON.stringify(blocked)); return }
              Promise.resolve(pool.run(url, (sess) => sess.invoke(url, tool, args == null ? {} : args, { confirm })))
                .then((out) => res.end(JSON.stringify(out)))
                .catch((err) => res.end(JSON.stringify({ ok: false, error: 'internal', message: String(err && err.message || err) })))
            })
          },
        }),
        hostCtx.webServer.register({
...
export { computeReadiness, aggregateReadiness } from './readiness.mjs'

export { NAME as name, VERSION as version }

import { computeReadiness as _computeReadiness, aggregateReadiness as _aggregateReadiness } from './readiness.mjs'
export default {
  name: NAME, version: VERSION, apply, resolveConfig, findChromium, isPrivateHostname, createSession,
  computeReadiness: _computeReadiness, aggregateReadiness: _aggregateReadiness,
}
