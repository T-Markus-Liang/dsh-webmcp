...
export function findChromium(cfg) {
  ...
  const execNames = process.platform === 'win32' ? ['chrome.exe', 'headless_shell.exe'] : ['chrome', 'headless_shell', 'Google Chrome for Testing']
  for (const root of roots) {
    if (!existsSync(root)) continue
    let builds = []
    try {
      builds = readdirSync(root).filter((d) => d.startsWith('chromium')).sort().reverse()
    } catch {
      continue
    }
...
function walkForExecutables(dir, names, depth) {
  ...
  for (const ent of entries) {
    ...
    if (ent.isDirectory()) {
      if (ent.name.endsWith('.app')) continue // descend manually below
...
class BrowserSession {
  ...
  async ensureBrowser(exeOverride) {
    ...
    else launchOpts.channel = 'chrome' // system Chrome, most robust last resort
    try {
      this.browser = await pw.chromium.launch(launchOpts)
    } catch (err) {
      const hint =
        `\n${NAME}: failed to launch Chromium.\n` +
        `Tried: ${exe ? `executablePath=${exe}` : 'channel=chrome (system Chrome)'}\n` +
        `Fix any one way:\n` +
