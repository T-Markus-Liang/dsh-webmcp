...
function failOf(err) {
  return {
    ok: false,
    error: err && err.name === 'TimeoutError' ? 'timeout' : 'internal',
  ...
  function discoverTool() {
    return {
      ...
      execute: async ({ url, refresh = false }) => {
        ...
        return session.discover(url, { refresh })
      },
    }
  }
  ...
  function invokeTool() {
    return {
      ...
      execute: async ({ url, tool, args, refresh = false }) => {
        const blocked = gateUrl(url)
        if (blocked) return blocked
        return session.invoke(url, tool, args, { refresh })
      },
    }
  }
...
export default { name: NAME, version: VERSION, apply, resolveConfig, findChromium, isPrivateHostname }
