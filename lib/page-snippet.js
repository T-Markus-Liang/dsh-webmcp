...
async function toolsFromMcImpl(mc) {
  if (!mc) return null;
...
(async () => {
  ...
  for (const { mc, surface } of (${COLLECT_MC_HOSTS_SRC})()) {
    const list = await (${toolsFromMcImpl.toString()})(mc);
      ...
      const findTool = async () => {
        ...
        try {
          for (const { mc, surface } of collectHosts()) {
            const list = await toolsFromMc(mc);
            if (!Array.isArray(list)) continue;
            const hit = list.find((t) => t && t.name === toolName);
            if (hit) {
              const schema = hit.inputSchema || null;
              if (typeof hit.execute === 'function') return { fn: hit.execute, surface, schema };
              if (typeof hit === 'function') return { fn: hit, surface, schema };
        ...
        try {
          ...
          if (w) {
            ...
            if (Array.isArray(list)) {
              ...
              if (hit) {
                const schema = hit.inputSchema || null;
                if (typeof hit === 'function') return { fn: hit, surface: 'window.webmcp', schema };
                if (typeof hit.execute === 'function') return { fn: hit.execute.bind(hit), surface: 'window.webmcp', schema };
                if (typeof hit.handler === 'function') return { fn: hit.handler, surface: 'window.webmcp', schema };
                if (typeof hit.fn === 'function') return { fn: hit.fn, surface: 'window.webmcp', schema };
              }
            }
          }
        } catch (_) {}
