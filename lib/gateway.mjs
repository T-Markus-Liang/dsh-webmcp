...
export function serve({ url, config = {}, session = null, input = process.stdin, output = process.stdout, manifestTtlMs = MANIFEST_TTL_MS } = {}) {
  ...
  async function listTools({ refresh = false } = {}) {
    ...
    stats.discovered += 1
    // v1.2.0: drift → push list_changed to a subscribed client.
    const namesNow = tools.map((t) => t.name).sort().join('\n')
    if (lastNames !== null && lastNames !== namesNow && toolsSubscribed) {
  ...
  async function handle(msg) {
    ...
    // Notifications are acknowledged by silence (no response), but the
    // subscription notifications still carry a side effect we must run.
